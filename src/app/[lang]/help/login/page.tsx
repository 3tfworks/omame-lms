...
// 初回ログインの 7 ステップ。w/h は実画像の寸法（step1/2/3/6/7・step5 は縦長、step4 は PC 横長）。
// step4（横長で小さい）と step5（縦長のメール本文）は ZoomableImage（タップ拡大）で表示。本文は step.n で出し分け。
const STEPS = [
  {
    n: 1,
  ...
  {
    n: 6,
    img: "/images/help/omame-login-confirm-sp.png",
    w: 390,
    h: 844,
    alt: "ログインの確認画面。「ログインを続ける」ボタンが表示されている",
...
const FAQS = [
  ...
  },
  {
    ...
    body: [
      "LINE のトーク画面からリンクを開いた場合も、確認画面で「ログインを続ける」を押せばログインできます。ただし、ログイン状態はLINE内の画面に保存され、普段お使いのSafariやChromeには引き継がれません。",
      "継続して受講しやすくするため、LINE 内ブラウザの右上にある「・・・」または「↗」ボタンを押し、「Safari で開く」または「Chrome で開く」を選んでから、「ログインを続ける」を押すことをおすすめします。",
    ],
  },
  {
      ...
      <div className="mx-auto max-w-2xl px-6 py-12 md:py-16">
        ...
        <div className="my-8 rounded-2xl bg-amber-50 p-5 border-l-4 border-amber-400" role="note">
          ...
          <p className="text-amber-900/90 text-sm md:text-base mt-2 leading-relaxed">
            ...
            をお選びください。普段お使いのブラウザにログイン状態が保存され、次回から受講ページを開きやすくなります。
          </p>
        </div>
        ...
        <section id="flow" className="mb-16 scroll-mt-6">
          ...
          <div className="space-y-4 text-base leading-relaxed md:text-lg">
            <p>
              ...
              あとは、そのメールのリンクを開き、確認画面で「ログインを続ける」を押すだけです。
            </p>
            <p>
                ...
                {step.n === 6 && (
                  <div className="space-y-3 text-base leading-relaxed md:text-lg">
                    <p>
                      ...
                      ボタンを押してください。この操作をしたブラウザでログインが完了します。
                    </p>
                  </div>
                )}
                {step.n === 7 && (
